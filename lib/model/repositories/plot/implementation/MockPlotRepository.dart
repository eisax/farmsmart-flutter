...
import 'package:farmsmart_flutter/model/entities/mock/mock_plot_seed.dart';
...
class MockPlotRepository implements PlotRepositoryInterface {
  ...
  MockPlotRepository._(
    ProfileRepositoryInterface profileRepository,
    IdentifyEntity<PlotEntity> identifyEntity,
    List<PlotEntity> startData,
  )   : this._profileRepository = profileRepository,
        this._identifyEntity = identifyEntity;
  ...
  Future<PlotEntity> addPlot(
      {Map<String, Map<String, String>>? plotInfo, CropEntity? crop}) {
  ...
  Future<PlotEntity> beginStage(PlotEntity forPlot, StageEntity stage) {
    forPlot = _replaceStage(
        forPlot, stage, _stageWithDates(stage, DateTime.now(), null));
  ...
  Future<PlotEntity> completeStage(PlotEntity forPlot, StageEntity stage) {
    forPlot = _replaceStage(
        forPlot, stage, _stageWithDates(stage, stage.started, DateTime.now()));
    return Future.value(forPlot);
  }
  ...
  Future<PlotEntity> revertStage(PlotEntity forPlot, StageEntity stage) {
    ...
    forPlot = _replaceStage(
        forPlot, stage, _stageWithDates(stage, stage.started, null));
    for (var i = stageIndex + 1; i < forPlot.stages.length; i++) {
      final upcomingStage = forPlot.stages[i];
      forPlot = _replaceStage(
          forPlot, upcomingStage, _stageWithDates(upcomingStage, null, null));
    }
    return Future.value(forPlot);
  }
  ...
  StageEntity _stageWithDates(
      StageEntity stage, DateTime? start, DateTime? end) {
    return StageEntity(
        id: stage.id, article: stage.article, started: start, ended: end);
  }
  ...
  Stream<PlotEntity> observeSingle(String uri) {
    final streamTransformer =
        StreamTransformer<PlotEntity, PlotEntity>.fromHandlers(
  ...
  MockListRepository<PlotEntity> _listFor(String id) {
    if (_listRepos[id] == null) {
      _listRepos[id] = MockListRepository<PlotEntity>(
          identifyEntity: _identifyEntity,
          startingData: MockPlotSeed.starterPlots());
  ...
  void _update({required String id, required List<PlotEntity> plots}) {
