...
class DocumentToPlotEntityTransformer
    extends ObjectTransformer<DocumentSnapshot<Map<String, dynamic>>, PlotEntity> {
  DocumentToPlotEntityTransformer();

  @override
  PlotEntity transform(
      {DocumentSnapshot? from, CropEntity? crop, List<StageEntity>? stages}) {
    final data = from?.data() as Map<String, dynamic>? ?? <String, dynamic>{};
    final title = castOrNull<String>(data[PlotEntityFields.title]) ?? '';
    final score = castOrNull<double>(data[PlotEntityFields.score]) ?? 0.0;
...
class PlotEntityToDocumentTransformer
  ...
  Map<String, dynamic> transform({PlotEntity? from}) {
    final source = from!;
    return {
      PlotEntityFields.title: source.title,
      PlotEntityFields.score: source.score,
      PlotEntityFields.cropPath: source.crop.uri,
      PlotEntityFields.stages: _transformStagesToFirebase(source),
    };
  }
  ...
  List<Map<String, dynamic>> _transformStagesToFirebase(PlotEntity from) {
    return from.stages.map((stage) {
      return {
        PlotEntityFields.id: stage.article
            .uri, //LH the stages need a id (in the plot context), since they are linked to the article 1 to 1 we can use the article uri
        PlotEntityFields.articlePath: stage.article.uri,
        PlotEntityFields.started:
            (stage.started != null) ? Timestamp.fromDate(stage.started!) : null,
        PlotEntityFields.ended:
            (stage.ended != null) ? Timestamp.fromDate(stage.ended!) : null,
      };
    }).toList();
  }
}
